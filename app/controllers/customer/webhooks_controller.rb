class Customer::WebhooksController < ApplicationController
  ...
  def create
    ...
    when 'checkout.session.completed'
      ...
      # トランザクション処理開始（処理の１つで例外が発生したら、複数の処理を巻き戻す）
  ...
  def create_order(session)
    # sessionを元に各カラムにデータを格納してcreateする（失敗時に例外処理をする）※ event.data.objectから取得
    Order.create!({
  ...
  def create_order_details(order, line_item)
    # Stripe に登録された商品を取得（product テーブルから購入された商品のデータを取得）
